...
function renderList(steps) {
  ...
  if (!steps.length) {
    ...
    empty.innerHTML = '<div class="title">No steps yet</div><div class="meta">Interact with any page and refresh.</div>';
...
// Optional auto-refresh while page is open
setInterval(() => {
  const exportArea = document.getElementById('exportArea');
  const exporting = !exportArea.classList.contains('hidden');
  if (!exporting) refresh();
  else updateRecordingButtons();
}, 1000);
